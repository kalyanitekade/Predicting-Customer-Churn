...
output = './output'
def run_model_and_plot_roc(model, X_train, X_test, y_train, y_test, w=None):
    ...
    if model == "Logistic":
        if w:
            log_reg = LogisticRegression(random_state=0, class_weight=w)
        else:
    ...
    elif model == "Logistic_balanced":
        log_reg_b = LogisticRegression(random_state=0, class_weight="balanced")
        log_reg_b.fit(X_train, y_train)
        y_pred = log_reg_b.predict(X_test)
        model_instance = log_reg_b
        ...
        print(f"Please choose one from (Logistic, Logistic_balanced, Random, Adaboost, Gradient)")
        return
